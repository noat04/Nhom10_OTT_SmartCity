...
const ChatInput = ({ onSendMessage, onTyping }) => {
  const [message, setMessage] = useState('');
  const [selectedFile, setSelectedFile] = useState(null);
  const [previewUrl, setPreviewUrl] = useState(null); // State để hiển thị ảnh thu nhỏ
  const [showEmoji, setShowEmoji] = useState(false);

  const fileInputRef = useRef(null);
  const typingTimeoutRef = useRef(null);
  const emojiRef = useRef(null); // Ref để bắt sự kiện click ra ngoài bảng emoji

  // 1. Dọn dẹp memory leak của Preview URL & Xử lý click ra ngoài bảng Emoji
  useEffect(() => {
  ...
  // 2. Xử lý khi người dùng gõ phím
  const handleChange = (e) => {
    setMessage(e.target.value);
    if (onTyping) {
      onTyping(true);
  ...
  const handleSend = () => {
    if (message.trim() || selectedFile) {
      // Phân biệt chính xác loại tin nhắn
      let msgType = 'text';
      if (selectedFile) {
  ...
  return (
    <div className="bg-white border-t border-gray-200 px-4 py-3 relative">
      ...
      {/* KHU VỰC PREVIEW FILE/ẢNH ĐÍNH KÈM */}
      {selectedFile && (
