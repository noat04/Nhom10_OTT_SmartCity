...
const userName = (user) => user?.fullName || user?.username || user?.email || "-";
const messageReceiverName = (message) => {
...
function Chart({ data = [], xLabel = "Ngày", yLabel = "Số lượng" }) {
...
function Table({ headers, children, scroll = false }) {
  return (
...
function UserDetail({ detail, onClose }) {
  return (
    <section className="admin-panel">
...
function GroupDetail({ detail, onClose }) {
  return (
    <section className="admin-panel">
      <h2>Chi tiết nhóm</h2>
      <div className="admin-detail">
